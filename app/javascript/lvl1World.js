...
function populateLevel_1() {
        ...
        if(selectedTile) {
            ...
            if( (Math.abs(selectedTile.x - targetBox1.x) <= closeEnough) && 
                ...
                if( ! selectedTile.dockedBox) {
                    dockedTotal += 1;
                    console.log("[] - DEBUG: "+selectedTile.name+" docked., Docked tiles: " + dockedTotal);
                }
                ...
                if(selectedTile.dockedBox) {
                    ...
                    dockedTotal -= 1;
                    console.log("[] - DEBUG: "+selectedTile.name+" un-docked. Docked tiles: " + dockedTotal);
                }
    ...
    //using pressup instead of mouseup because of "https://createjs.com/tutorials/Mouse%20Interaction/"
    worldStage.on("pressup", function(evt) {
        
        //logOnce = false;
        if(dockedTotal == 8 && logOnce == false) {
            //record to the database this action
            log2dblvl1("solution",""+selectedTile.name+" moved. 8 tiles in place.");
            logOnce = true;
        }
        else if(dockedTotal < 8 && logOnce == false) {
            log2dblvl1("attempt",""+selectedTile.name+" moved. "+dockedTotal+" tiles in place.");
            logOnce = true;
        }
        selectedTile = null;
    })
...
function getTileInBox(box) {
    //loop through all tiles to find which one is docked in 'box'
    var tempTile = null;
    
}
function log2dblvl1(cat, det) {
    $.ajax("/useractions", {type: "POST", async: true, data: {level: "level1", category: cat, details: det}})
}
